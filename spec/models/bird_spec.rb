...
describe Bird, type: :model do
  subject { FactoryBot.create :bird }
  ...
  describe '#set_big_year' do
    ...
    context 'with user who is participant of BigYear' do
      let!(:user_subscription) { FactoryBot.create :subscription, user: user, year: current_year }

      context 'with timestamp' do
        subject { FactoryBot.create :bird, user: user, timestamp: Time.zone.now }
      ...
      context 'without timestamp' do
        subject { FactoryBot.create :bird, user: user }
  ...
  describe '#update_big_year' do
    ...
    context "when bird wasn't participant of BigYear" do
      subject { FactoryBot.create :bird, user: user, timestamp: 1.year.ago }
